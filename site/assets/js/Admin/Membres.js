    ...
    request.done(function (msg) {
        ...
        for (let i = 0; i < (msg.length - 1); i++) {
            ...
            $("#dataTable").append("" +
                ...
                "                                   <button class=\"btn btn-primary\" type=\"button\" style=\"display: none; height: 30px;font-size: 12px;\">Confirmer</button> </td>\n" +
                "                                </tr>")
...
//Si l'on clique sur le bouton de suppression
            $("#" + msg[i].ID + " div").click(function () {
                //Afficher un message de confirmation
                if (window.confirm("Voulez-vous supprimer ce membre ?")) {
                    //Si la confirmation est positive alors lancer la requête
                    request = $.ajax({
                        method: "POST",
                    ...
                    request.done(function (msg3) {
                        //Si la suppression à bien été effectué
    ...
    $("#Ajouter-un-membre").click(function () {
        ...
        var request = $.ajax({
            method: "GET",
            url: "http://51.210.151.13/btssnir/projets2022/fablab/api/carte/libre.php",
            data: {},
            dataType: "json"
        });
        request.done(function (msg) {
            ...
            for (let i = 0; i < msg.length; i++) {
                $("#Carte-Nouveau").append(`<option value="${msg[i].iduid}">${msg[i].iduid}</option>`)
            }
        })
